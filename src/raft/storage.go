...
func (ms *MemoryStorage) LastIndex() (int, error) {
	ms.Lock()
	defer ms.Unlock()
	return ms.snapshot.Metadata.Index + int(len(ms.ents)) - 1, nil
}
...
func (ms *MemoryStorage) Compact(i int, cs *ConfState, data []byte) error {
	ms.Lock()
	defer ms.Unlock()
	offset := ms.snapshot.Metadata.Index
	if i <= offset {
	...
	if i > offset+int(len(ms.ents))-1 {
		log.Panicf("compact %d is out of bound lastindex(%d)", i, offset+int(len(ms.ents))-1)
	}
	i -= offset
	ents := make([]Entry, 1, 1+int(len(ms.ents))-i)
...
func (ms *MemoryStorage) Append(entries []Entry) error {
	...
	last := entries[0].Index + int(len(entries)) - 1
	...
	default:
		log.Panicf("missing log entry [last: %d, append at: %d]",
			ms.snapshot.Metadata.Index+int(len(ms.ents)), entries[0].Index)
	}
	return nil
}
