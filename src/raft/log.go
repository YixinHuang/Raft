...
func (l *raftLog) maybeAppend(index, logTerm, committed int, ents ...Entry) (lastnewi int, ok bool) {
	lastnewi = index + int(len(ents))
...
func (l *raftLog) findConflict(ents []Entry) int {
	for _, ne := range ents {
...
func (l *raftLog) commitTo(tocommit int) {
...
func (l *raftLog) stableTo(i, t int) { l.unstable.stableTo(i, t) }

func (l *raftLog) stableSnapTo(i int) { l.unstable.stableSnapTo(i) }

func (l *raftLog) lastTerm() int { return l.term(l.lastIndex()) }
...
func (l *raftLog) entries(i int) []Entry {
	if i > l.lastIndex() {
		return nil
	}
...
func (l *raftLog) isUpToDate(lasti, term int) bool {
	return term > l.lastTerm() || (term == l.lastTerm() && lasti >= l.lastIndex())
}

func (l *raftLog) matchTerm(i, term int) bool { return l.term(i) == term }

func (l *raftLog) maybeCommit(maxIndex, term int) bool {
	if maxIndex > l.committed && l.term(maxIndex) == term {
		DPrintf("[maybeCommit@log.go2B] return True, maxIndex:=[%d] l.committed:=[%d]  l.term(%d):=[%d] term:=[%d]", maxIndex, l.committed, maxIndex, l.term(maxIndex), term)
		l.commitTo(maxIndex)
		return true
	}
	DPrintf("[maybeCommit@log.go2B] return False, maxIndex:=[%d] l.committed:=[%d]  l.term(%d):=[%d] term:=[%d]", maxIndex, l.committed, maxIndex, l.term(maxIndex), term)
	return false
}
...
func (l *raftLog) slice(lo int, hi int) []Entry {
...
func (l *raftLog) mustCheckOutOfBounds(lo, hi int) {
	if lo > hi {
